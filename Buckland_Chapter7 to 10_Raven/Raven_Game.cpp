...
bool Raven_Game::AttemptToAddBot(Raven_Bot* pBot)
{
  ...
  while (--attempts >= 0)
  { 
    //select a spawn point with the bot type
	  Vector2D pos;
	  pos = m_pMap->GetSpawnPoints()[pBot->my_type];
...
{ 
  while (NumBotsToAdd--)
  {
	...
	Raven_Bot* rb = new Raven_Bot(this,Vector2D());
	Raven_Bot_Plus* rbp = new Raven_Bot_Plus(this,Vector2D());
